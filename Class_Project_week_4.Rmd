...
The first model is a classification tree (method = "rpart") using all predictors.
```{r model_1, echo=TRUE  }
...
model_1         <- train(as.factor(classe) ~ . ,  method = "rpart", data = trainingData)
        ...
        plot(model_1$finalModel, uniform=TRUE, main="Classification Tree")
        text(model_11$finalModel, use.n = TRUE, all = TRUE, cex = 0.8)
...
model_2         <- train(as.factor(classe) ~ accel_belt_y + accel_belt_z + magnet_belt_x + magnet_belt_y + accel_dumbbell_z + magnet_dumbbell_x + magnet_dumbbell_y + magnet_dumbbell_z + roll_forearm + pitch_forearm + yaw_forearm + total_accel_forearm + gyros_forearm_x , method = "rpart", data = trainingData)
        ...
        text(model_2$finalModel, use.n = TRUE, all = TRUE, cex = 0.8)
...
```{r, prediction_1_2, echo=TRUE}
        ...
        predict(model_1, testingData)
        predict(model_2, testingData)
...
My_Confusion    <- confusionMatrix(validate$classe, val_predict)
        
        My_Confusion$overall[3]
        My_Confusion[2]
...
## Conclusions
The random Forest model out performed the Classification Tree model by leaps and bounds. The results are only based on the data set the models were trained on and it is expected the results would be different for data collected under different circumstances and the individuals who performed the exercises.
